...
/**
 * Custom Job Status holding a breakage detector plan.
 *
 * @version $Id: $
 * @since 0.7
 */
public class HierarchyBreakageDetectorJobStatus extends DefaultJobStatus<MigrationPlanRequest>
{
    private List<Breakage> breakages;

    /**
     * @param request the request provided when started the job
     * @param parentJobStatus the status of the parent job (i.e. the status of the job that started this one); pass {@code
     * null} if this job hasn't been started by another job (i.e. if this is not a sub-job)
     * @param observationManager the observation manager component
     * @param loggerManager the logger manager component
     */
    public HierarchyBreakageDetectorJobStatus(MigrationPlanRequest request, JobStatus parentJobStatus,
